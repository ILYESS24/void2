    ...
    if [ ! -d "node_modules/$pkg" ] || ! node -e "require.resolve('$pkg')" 2>/dev/null; then
        ...
        case "$pkg" in
            "merge-options")
                ...
                ;;
                "copy-webpack-plugin")
                    npm install copy-webpack-plugin@^10.2.4 --legacy-peer-deps --save-prod --force --ignore-scripts 2>&1 | tail -10 || echo "⚠️ Installation copy-webpack-plugin échouée"
                    ;;
            "path-browserify")
